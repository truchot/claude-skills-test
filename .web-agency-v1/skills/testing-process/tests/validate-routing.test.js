...
reporter.header('Validating Testing Process Routing Keywords');
...
if (skillContent.includes('Routing') || skillContent.includes('Routage') || skillContent.match(/Règles?\s+de\s+Routage/i)) {
...
if (skillContent.includes('Arbre de Décision') || skillContent.includes('Decision Tree')) {
  reporter.pass('SKILL.md contains decision tree');
} else {
  reporter.warn('SKILL.md may not have a decision tree');
}
...
// Check for skill relationships
reporter.section('Skill Relationships');
...
for (const skill of relatedSkills) {
  if (combinedContent.includes(skill.toLowerCase())) {
    reporter.pass(`${skill} relationship documented`);
  } else {
    reporter.warn(`${skill} not mentioned in documentation`);
  }
}
