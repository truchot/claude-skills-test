#!/usr/bin/env node
/**
 * Test: Validate UX/UI Design Agent Structure
 *
...
const { SKILL_ROOT, DOMAINS, AGENT_REQUIREMENTS, EXPECTED_AGENTS_PER_DOMAIN } = require('./config');
...
function validateAgent(filePath) {
  ...
  // Check expected sections for orchestrators
  if (isOrchestrator) {
...
// Main execution
reporter.header('Validating UX/UI Design Agent Structure');
...
for (const agents of Object.values(EXPECTED_AGENTS_PER_DOMAIN)) {
...
for (const domain of DOMAINS) {
  ...
  for (const agentName of expectedAgents) {
    const agentPath = path.join(domainDir, `${agentName}.md`);
    const { content, error } = safeReadFile(agentPath);

    if (error) {
      reporter.fail(`${domain}/${agentName}.md: File not found`);
      continue;
    }

    totalAgents++;
    const relativePath = `agents/${domain}/${agentName}.md`;
    const { errors, warnings: fileWarnings } = validateAgent(agentPath);
...
reporter.info(`Expected agents: ${expectedTotal}, Validated: ${totalAgents}`);
